                                    ...
                                    <div class="card-header flex-wrap py-5">
                                        ...
                                        <div class="card-toolbar">
                                            ...
                                            if (hasRight($user_right_title, 'add')) {
                                                echo '<a href="' . $admin_url . 'start_evaluation" class="btn btn-primary font-weight-bolder"><i class="la la-plus"></i>New Record</a>';
                                            }
                                            ?>
                                        </div>
                                    </div>
                                            ...
                                            <div class="row align-items-center">
                                                <div class="col-lg-12 col-xl-12">
                                                    <div class="row align-items-center">
                                                        ...
                                                        <div class="col-md-3 my-2 my-md-0">
                                                            <div class="form-group">
                                                                <label for="BG_EvaluationTypeIdFilter">Evaluation
                                                                    Type</label>
                                                                <select class="form-control"
                                                                        id="BG_EvaluationTypeIdFilter"
                                                                        onchange="getData()">
...
<script>
    ...
    function getData() {
        ...
        var BG_EvaluationTypeIdFilter = document.getElementById('BG_EvaluationTypeIdFilter');
        ...
        var filter = {
            ...
            'Filter': [
                {'field': 'ev.department_id', 'value': BG_DepartmentFilter.value},
                {'field': 'ev.evaluation_type_id', 'value': BG_EvaluationTypeIdFilter.value},
            ],
    ...
    function getAllEvaluations(filter) {
        loader(true);
        $.ajax({
            type: "POST", url: 'ajax/fetch/evaluations.php',
            data: {'filters': filter},
    ...
    function SendEmail(url, appraise, appraiser) {
        loader(true);
        var postData = {"url": url, "appraise": appraise, "appraiser": appraiser};
        $.ajax({
            type: "POST", url: "ajax/common.php",
            data: {'postData': postData, 'sendEvaluationMail': true},
            success: function (resPonse) {
                if (resPonse !== undefined && resPonse != '') {
                    var obj = JSON.parse(resPonse);
                    if (obj.code !== undefined && obj.code != '') {
                        if (obj.code === 200) {
                            ...
                            if (obj.accessDeniedMessage !== undefined && obj.accessDeniedMessage != '') {
                                toasterTrigger('error', obj.accessDeniedMessage);
                            }
                            loader(false);
                        } else {
                            loader(false);
    ...
    function removeEmployeeFromEvaluation(ev_id, evd_id) {
        Swal.fire({
            title: 'Are you sure want to delete?',
            text: "You won't be able to revert this!",
            icon: 'warning',
            ...
            if (result.value) {
                ...
                $.ajax({
                    type: "POST", url: "ajax/delete.php",
                    data: " remove_employee_from_evaluation=" + evd_id + "&evaluation_id=" + ev_id,
                    success: function (resPonse) {
                        if (resPonse !== undefined && resPonse != '') {
                            var obj = JSON.parse(resPonse);
                            if (obj.code === 200 || obj.code === 405 || obj.code === 422) {
