                                        ...
                                        <div class="page_wrapper">
                                                ...
                                                <div class="col-md-10">
                                                    <div class="row font-size-h5 my-6">
                                                        ...
                                                        <div class="col-md-3 text-overflow-ellipsis">
                                                            <b>Grade
                                                                : </b> <?php echo $appraiseInfo->salary_grade_name; ?>
                                                        </div>
                                                    </div>
                                                    ...
                                                    <div class="modal-dialog modal-dialog-centered modal-lg"
                                                        ...
                                                        <div class="modal-content">
                                                            <div class="modal-header">
                                                                <h5 class="modal-title" id="exampleModalLabel">Scores
                                                                    Detail</h5>
                                            ...
                                            if (mysqli_num_rows($query_evaluation_tasks) > 0) {
                                                while ($fetch_evaluation_tasks = mysqli_fetch_object($query_evaluation_tasks)) {
                                                    ...
                                                    <?php
                                                    $select_evaluation_task_details = "SELECT `id`,`task_description`, `total_number` FROM `evaluation_task_details` WHERE `evaluation_task_id`='{$fetch_evaluation_tasks->id}' ORDER BY `id` ASC";
                                                    ...
                                                    echo '</div><p class="overflow-hidden">';
                                                    echo '<input type="hidden" class="not-display" name="r_row[]" id="r_row_' . $section . '" value="' . $row . '">';
                                                    echo '<button type="button" class="btn btn-outline-primary mr-1 mt-3 btn-sm float-right" onclick="addNewRow(' . $section . ', ' . $task_weight . ', ' . $fetch_evaluation_tasks->id . ')">Add</button>';
                                                    echo '</p></div></div>';
                                                }
                                                ?>
                                                <div class="row mb-3">
...
?>
    <script>
        ...
        function addNewRow(section, weight, task_id) {
            ...
            innerHTml_c += '<input type="checkbox" checked class="lineRepresentativeBox" value="' + id + '" id="lineRepresentativeBox_' + id + '" onclick="calculate(event)" data-portion="' + section + '" data-line="' + row + '" data-task_weight="' + weight + '"/>';
            ...
            innerHTml_c += '<input type="hidden" id="old_measure_number_' + id + '" value="0.00">';
            innerHTml_c += '<select id="measure_number_' + id + '" <?php echo $onblur; ?> class="form-control" onchange="setMeasure(this.value, ' + section + ', ' + row + ')">';
            innerHTml_c += '<option value="0" selected="selected">Select</option>';
            <?php
            foreach (getGreaterThanNumberStacks($number_stacks['stack'], '1') as $score_stacks) {
            ...
            innerHTml_c += '<select id="obtaining_number_' + id + '" <?php echo $onblur; ?> class="form-control" onchange="setObtainingPercentages(this.value, ' + section + ', ' + row + ', ' + weight + ')">';
            innerHTml_c += '<option value="0" selected="selected">Select</option>';
            <?php
