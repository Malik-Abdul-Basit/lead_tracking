                                ...
                                <div class="card-header flex-wrap py-5">
                                    ...
                                    <div class="card-toolbar">
                                        ...
                                        if (hasRight($user_right_title, 'add')) {
                                            echo '<a href="' . $admin_url . 'team" class="btn btn-primary font-weight-bolder"><i class="la la-plus"></i>New Record</a>';
                                        }
                                        ?>
                                    </div>
                                </div>
                                                        ...
                                                        <div class="form-group">
                                                            ...
                                                            <select class="form-control" id="employee_id"
                                                                ...
                                                                $select = "SELECT emp.id, emp.employee_code, CONCAT(empb.first_name,' ',empb.last_name) AS full_name
                                                                        ...
                                                                        INNER JOIN 
                                                                            `teams` AS `t` 
                                                                        ON `emp`.`id` = `t`.`employee_id`
                                                                        WHERE `emp`.`company_id`='{$global_company_id}' AND `emp`.`branch_id`='{$global_branch_id}' 
                                                                        AND `emp`.`status`='{$active}' AND `emp`.`deleted_at` IS NULL GROUP BY `emp`.`id` ORDER BY `emp`.`employee_code` ASC";
                                                                $query = mysqli_query($db, $select);
                                                                if (mysqli_num_rows($query) > 0) {
                                                                    while ($result = mysqli_fetch_object($query)) {
                                                                        ?>
                                                                        <option value="<?php echo $result->id; ?>"><?php echo $result->employee_code . ' - ' . $result->full_name; ?></option>
                                                                        <?php
                                                                    }
                                                                }
                                                                ?>
...
?>
    <script>
        ...
        function getData() {
            ...
            var BG_SortColumn = document.getElementById('BG_SortColumn');
            var BG_SortOrder = document.getElementById('BG_SortOrder');
            var BG_EmployeeFilter = document.getElementById('employee_id');
            ...
            var filter = {
                ...
                'Filter': [
                    {'field': 't.employee_id', 'value': BG_EmployeeFilter.value},
                ],
                'Numbering': {
                    'field': 'sr',
                    'title': '#',
        ...
        function getAllTeams(filter) {
            loader(true);
            $.ajax({
                type: "POST", url: 'ajax/fetch/teams.php',
                data: {'filters': filter},
