                                            ...
                                            <h3 class="card-title">
                                                ...
                                                if (isset($_GET['id'])) {
                                                    ...
                                                    if (numberStackUsed($id, $default)) {
                                                        ...
                                                        $Q = "SELECT `id`, `department_id` FROM `evaluation_default_number_stacks` WHERE `id`='{$id}' AND `company_id`='{$global_company_id}' AND `branch_id`='{$global_branch_id}' AND `deleted_at` IS NULL";
                                                    ...
                                                    <div class="mb-2">
                                                        <div id="Data_Holder_Parent_Div">
                                                            ...
                                                            <div id="Data_Holder_Child_Div" class="mt-7 mb-7">
                                                                ...
                                                                $i = 1;
                                                                if (!empty($id)) {
                                                                    $query_gpd = mysqli_query($db, "SELECT * FROM `evaluation_default_number_stack_details` WHERE `gp_id`='{$id}' ORDER BY `gp_value` DESC");
...
?>
    <script>
        function saveFORM() {
            ...
            department_id.style.borderColor = select2_department_id_container.style.borderColor = '#E4E6EF';
            errorMessageDepartment.innerText = responseMessage.innerText = '';
            responseMessageWrapper.style.display = "none";

            var checkedValue = null;
            var continueProcessing = false;
            var data = [];
            var message = 'Please checked at least one Stack Detail row.';
            ...
            if (id.value == 0 && A == '') {
                ...
                return false;
            } else if (isNaN(department_id.value) === true || department_id.value.length > 10) {
                select2_department_id_container.style.borderColor = '#F00';
                ...
                if (continueProcessing === false) {
                    ...
                    $.ajax({
                        type: "POST", url: "ajax/default_number_stack.php",
