                                        ...
                                        <div class="card-header">
                                            <h3 class="card-title">
                                                ...
                                                if (isset($_GET['id'])) {
                                                    ...
                                                    $Q = "SELECT * FROM `shifts` WHERE `id`='{$id}' AND `company_id`='{$global_company_id}' AND `branch_id`='{$global_branch_id}' AND `deleted_at` IS NULL";
                                                    ...
                                                    if ($Rows != 1) {
                                                        ...
                                                        $from = html_entity_decode(stripslashes(date('h:i A', strtotime($Result->from))));
                                                        $to = html_entity_decode(stripslashes(date('h:i A', strtotime($Result->to))));
...
?>
    <script>
        function saveFORM() {
            ...
            var from = document.getElementById('shift_from');
            var to = document.getElementById('shift_to');
            var sort_by = document.getElementById('sort_by');
            var checkValidTime12 = /^(((0[1-9])|(1[0-2])):([0-5])([0-9])\s(A|P)M)?$/;
            //var checkValidTime24 = /^([0-1]?[0-9]|2[0-4]):([0-5][0-9])(:[0-5][0-9])?$/;
            ...
            name.style.borderColor = from.style.borderColor = to.style.borderColor = sort_by.style.borderColor = '#E4E6EF';
            errorMessageName.innerText = errorMessageFrom.innerText = errorMessageTo.innerText = errorMessageSortBy.innerText = "";
            ...
            if (id.value == 0 && A == '') {
                ...
                errorMessageFrom.innerText = "From field is required.";
                return false;
            } else if (!checkValidTime12.test(from.value) || from.value.length != 8) {
                from.style.borderColor = '#F00';
                errorMessageFrom.innerText = "Please select a valid time.";
                return false;
            } else if (to.value == '') {
                to.style.borderColor = '#F00';
                errorMessageTo.innerText = "To field is required.";
                return false;
            } else if (!checkValidTime12.test(to.value) || from.value.length != 8) {
                to.style.borderColor = '#F00';
                errorMessageTo.innerText = "Please select a valid time.";
                return false;
                ...
                sort_by.style.borderColor = '#F00';
                errorMessageSortBy.innerText = "Length should not exceed 9 characters.";
                return false;
            } else {
                loader(true);
                var postData = {
