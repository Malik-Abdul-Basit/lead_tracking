                                ...
                                <div class="flex-row-fluid ml-lg-8">
                                    ...
                                    <div class="card card-custom card-stretch">
                                        ...
                                        <div class="card-header py-3">
                                            <?php
                                            $sql_payroll = mysqli_query($db, "SELECT * FROM `employee_payrolls` WHERE `employee_id`='{$result->employee_id}' ORDER BY `joining_date` DESC LIMIT 1");
                                            ?>
...
?>
    <script>
        ...
        function getData() {
            ...
            loader(true);
            $.ajax({
                type: "POST", url: 'ajax/fetch/payroll_overview.php',
                data: {'filters': filter},
        ...
        function entryDelete(id, emp) {
            Swal.fire({
                ...
                if (result.value) {
                    var D = '<?php echo hasRight('employee_payroll', 'delete') ?>';
                    if (D) {
                        loader(true);
                        $.ajax({
                            type: "POST", url: "ajax/delete.php",
                            data: "delete_employee_payroll=" + id + "&emp=" + emp,
                            success: function (resPonse) {
                                if (resPonse !== undefined && resPonse != '') {
                                    var obj = JSON.parse(resPonse);
