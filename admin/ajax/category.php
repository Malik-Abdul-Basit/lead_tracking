...
if (isset($_POST['postData'])) {
    ...
    if ((empty($id) || $id == 0) && (!hasRight($user_right_title, 'add'))) {
        ...
        $checkExist = mysqli_query($db, "SELECT `id` FROM `categories` WHERE `name`='{$name}' AND `company_id`='{$company_id}' AND `branch_id`='{$branch_id}' AND `id`!='{$id}' AND `deleted_at` IS NULL");
        if (mysqli_num_rows($checkExist) > 0) {
            echo json_encode(["code" => 405, "toasterClass" => 'error', "responseMessage" => 'Record already exist.']);
        } else {
            if (!empty($id) && $id > 0) {
                $query = "UPDATE `categories` SET `name`='{$name}',`sort_by`='{$sort_by}',`updated_by`='{$user_id}' WHERE `id`='{$id}'";
                $form_reset = false;
            } else {
                $query = "INSERT INTO `categories`(`id`, `name`, `sort_by`, `company_id`, `branch_id`, `added_by`) VALUES (NULL, '{$name}', '{$sort_by}', '{$company_id}', '{$branch_id}', '{$user_id}')";
                $form_reset = true;
            }
