...
if (isset($_POST['postData'])) {
    ...
    if ((empty($id) || $id == 0) && (!hasRight($user_right_title, 'add'))) {
        ...
        echo json_encode(['code' => 422, 'errorField' => 'category_id', 'errorDiv' => 'errorMessageCategory', 'errorMessage' => 'Department field is required.']);
    } else if (!is_numeric($category_id) || strlen($category_id) > 10) {
        echo json_encode(['code' => 422, 'errorField' => 'category_id', 'errorDiv' => 'errorMessageCategory', 'errorMessage' => 'Please select a valid option.']);
        ...
        $checkExist = mysqli_query($db, "SELECT `id` FROM `sub_categories` WHERE `name`='{$name}' AND `category_id`='{$category_id}' AND `company_id`='{$company_id}' AND `branch_id`='{$branch_id}' AND `id`!='{$id}' AND `deleted_at` IS NULL");
        if (mysqli_num_rows($checkExist) > 0) {
            echo json_encode(["code" => 405, "toasterClass" => 'error', "responseMessage" => 'Sub Category already exist.']);
        } else {
            if (!empty($id) && $id > 0) {
                $query = "UPDATE `sub_categories` SET `name`='{$name}',`category_id`='{$category_id}',`sort_by`='{$sort_by}',`updated_by`='{$user_id}' WHERE `company_id`='{$company_id}' AND `branch_id`='{$branch_id}' AND `id`='{$id}'";
                $form_reset = false;
            } else {
                $query = "INSERT INTO `sub_categories`(`id`, `name`, `category_id`, `sort_by`, `company_id`, `branch_id`, `added_by`) VALUES (NULL, '{$name}', '{$category_id}', '{$sort_by}', '{$company_id}', '{$branch_id}', '{$user_id}')";
                $form_reset = true;
            }
