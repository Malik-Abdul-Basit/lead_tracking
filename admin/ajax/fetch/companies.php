...
if (isset($_POST['filters']) && !empty($_POST['filters'])) {
    ...
    if (isset($filters->SearchQuery) && $filters->SearchQuery != '' && strlen($filters->SearchQuery) > 0) {
        $str = $filters->SearchQuery;
        if (strtolower($str) == 'close') {
            $str = '0';
        } else if (strtolower($str) == 'working') {
            $str = '1';
        }
        $condition .= " AND (`id` LIKE '%{$str}%' OR `name` LIKE '%{$str}%' OR `status` LIKE '%{$str}%') ";
    }

    $total = 0;
    $data = '';
    $sql = mysqli_query($db, "SELECT count(id) AS total FROM `companies`" . $condition);
    ...
    if ($total > 0) {
        ...
        $select = "SELECT `id`,`name`,`status` FROM `companies` " . $condition . $sort . $number_of_record;
