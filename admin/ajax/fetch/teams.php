...
if (isset($_POST['filters']) && !empty($_POST['filters'])) {
    ...
    $sortColumn = 'sort_by';
    $sortOrder = 'ASC';
    $condition = " WHERE t.company_id='{$global_company_id}' AND t.branch_id='{$global_branch_id}' AND t.deleted_at IS NULL AND e.company_id='{$global_company_id}' AND e.branch_id='{$global_branch_id}' ";
    if (isset($filters->SearchQuery) && !empty($filters->SearchQuery) && strlen($filters->SearchQuery) > 0) {
        $condition .= " AND (t.id LIKE '%{$filters->SearchQuery}%' OR t.name LIKE '%{$filters->SearchQuery}%') ";
    }
    ...
    $total = 0;
    $data = '';
    $sql = mysqli_query($db, "SELECT count(t.id) AS total FROM teams AS t INNER JOIN employees AS e ON t.employee_id = e.id" . $condition);
    ...
    if ($total > 0) {
        ...
        $select = "SELECT t.id, t.name, t.sort_by, CONCAT(e.employee_code,' - ',empb.first_name,' ',empb.last_name) AS employee
        FROM
            teams AS t 
        INNER JOIN
            employees AS e 
        ...
        ON t.employee_id = empb.employee_id" . $condition . $sort . $number_of_record;
                    ...
                    if (hasRight($filters->L, 'edit') || hasRight($filters->L, 'delete')) {
                        if (isset($filters->Actions) && !empty($filters->Actions) && sizeof($filters->Actions) > 0) {
                            ...
                            if (hasRight($filters->L, 'edit'))
                                $data .= '<a href="' . $admin_url . 'team?id=' . $result['id'] . '" class="btn btn-sm btn-clean btn-icon" title="Edit"><i class="la la-edit"></i></a>';
