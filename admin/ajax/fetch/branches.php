...
if (isset($_POST['filters']) && !empty($_POST['filters'])) {
    ...
    if (isset($filters->SearchQuery) && !empty($filters->SearchQuery) && strlen($filters->SearchQuery) > 0) {
        $condition .= " AND (b.name LIKE '%{$filters->SearchQuery}%' OR b.company_email LIKE '%{$filters->SearchQuery}%' OR b.hr_email LIKE '%{$filters->SearchQuery}%' OR b.other_email LIKE '%{$filters->SearchQuery}%') ";
    }
    ...
    $total = 0;
    $data = '';
    $sql = mysqli_query($db, "SELECT count(b.id) AS total FROM branches AS b INNER JOIN countries AS c ON b.country_id = c.id INNER JOIN states AS s ON b.state_id = s.id INNER JOIN cities AS ci ON b.city_id = ci.id " . $condition);
    ...
    if (isset($filters->L) && !empty($filters->L)) {
    ...
    if ($total > 0) {
        ...
        INNER JOIN 
            cities AS ci 
            ON b.city_id = ci.id " . $condition . $sort . $number_of_record;
        ...
        if (mysqli_num_rows($query) > 0) {
            ...
            while ($result = mysqli_fetch_assoc($query)) {
                ...
                foreach ($filters->Header as $key => $value) {
                    ...
                    $f = $c->field;
                    if ($f == 'type') {
                        $innerHTML = config('branches.type.label.' . $result[$f]);
                    } else if ($f == 'status') {
                        $innerHTML = config('branches.status.label.' . $result[$f]);
                    } else {
                        $innerHTML = $result[$f];
                    }
                    ...
                    if (hasRight($filters->L, 'edit') || hasRight($filters->L, 'delete')) {
                        if (isset($filters->Actions) && !empty($filters->Actions) && sizeof($filters->Actions) > 0) {
                            ...
                            if (hasRight($filters->L, 'edit'))
                                $data .= '<a href="' . $admin_url . 'branch?id=' . $result['id'] . '" class="btn btn-sm btn-clean btn-icon" title="Edit"><i class="la la-edit"></i></a>';
