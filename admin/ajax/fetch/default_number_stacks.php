...
if (isset($_POST['filters']) && !empty($_POST['filters'])) {
    ...
    if (isset($filters->SearchQuery) && !empty($filters->SearchQuery) && strlen($filters->SearchQuery) > 0) {
        $condition .= " AND (gp.id LIKE '%{$filters->SearchQuery}%' OR dep.name LIKE '%{$filters->SearchQuery}%') ";
    }
    ...
    $total = 0;
    $data = '';
    $sql = mysqli_query($db, "SELECT count(gp.id) AS total FROM evaluation_default_number_stacks AS gp LEFT JOIN departments AS dep ON dep.id=gp.department_id " . $condition);
                    ...
                    <div class="collapse-card-outer-wrapper">
                        <div class="table-header">
                            <div class="row">
                                ...
                                <div class="col-md-8"><b>Department</b></div>
    ...
    if ($total > 0) {
        ...
        $select = "SELECT gp.id, dep.name
        FROM
            evaluation_default_number_stacks AS gp
         LEFT JOIN 
            departments AS dep
            ON dep.id=gp.department_id " . $condition . $sort . $number_of_record;
        ...
        if (mysqli_num_rows($query) > 0) {
            ...
            while ($result = mysqli_fetch_object($query)) {
                    ...
                    <div class="collapse-card">
                        <div class="card-pane success">
                            <div class="row">
                                ...
                                <div class="col-md-8 text-vertical-align-center">' . $department_name . '</div>
                                <div class="col-md-2 text-vertical-align-center">';
                                    if (!numberStackUsed($result->id, $default) && isset($filters->L) && !empty($filters->L) && (hasRight($filters->L, 'edit') || hasRight($filters->L, 'delete'))) {
                                        if (hasRight($filters->L, 'edit')) {
                                            $data .= '<a href="' . $admin_url . 'default_number_stack?id=' . $result->id . '" class="btn btn-sm btn-light-primary font-weight-bolder text-uppercase mr-2" style="font-size: 10px" title="Edit">
                ...
                $query_gpd = mysqli_query($db, "SELECT * FROM `evaluation_default_number_stack_details` WHERE `gp_id`='{$result->id}' ORDER BY `gp_value` DESC");
