...
if (isset($_POST['filters']) && !empty($_POST['filters'])) {
    ...
    if (isset($filters->SearchQuery) && !empty($filters->SearchQuery) && strlen($filters->SearchQuery) > 0) {
        $condition .= " AND (gp.id LIKE '%{$filters->SearchQuery}%') ";
    }
    ...
    $total = 0;
    $data = '';
    $sql = mysqli_query($db, "SELECT count(gp.id) AS total FROM evaluation_grading_policies AS gp INNER JOIN evaluation_types AS evt ON gp.evaluation_type_id=evt.id INNER JOIN salary_grades AS sg ON gp.salary_grade_id=sg.id LEFT JOIN departments AS dep ON gp.department_id=dep.id " . $condition);
    ...
    if (isset($filters->L)) {
        if (hasRight($filters->L, 'edit') || hasRight($filters->L, 'delete')) {
            $h_col = '<div class="col-md-3"><b>Department</b></div><div class="col-md-3"><b>Salary Band</b></div><div class="col-md-2"><b>Action</b></div>';
            $right = true;
        } else {
            $h_col = '<div class="col-md-4"><b>Department</b></div><div class="col-md-4"><b>Salary Band</b></div>';
            $right = false;
        }
    }
                    ...
                    <div class="collapse-card-outer-wrapper">
                        <div class="table-header">
                            <div class="row">
                                ...
                                <div class="col-md-3"><b>Evaluation Type</b></div>' . $h_col . '
                            </div>
                        </div>
                    </div>
                </th>
    ...
    if ($total > 0) {
        ...
        $select = "SELECT gp.id,
        evt.name AS evaluation_types_name,
        dep.name AS department_name,
        sg.name AS salary_grade_name
        FROM 
            evaluation_grading_policies AS gp 
        INNER JOIN 
            evaluation_types AS evt 
            ON gp.evaluation_type_id=evt.id 
        INNER JOIN salary_grades AS sg 
            ON gp.salary_grade_id=sg.id 
        LEFT JOIN 
            departments AS dep 
            ON gp.department_id=dep.id " . $condition . $sort . $number_of_record;
        ...
        if (mysqli_num_rows($query) > 0) {
            ...
            while ($result = mysqli_fetch_object($query)) {
                    ...
                    <div class="collapse-card">
                        <div class="card-pane success">
                            <div class="row">
                                ...
                                <div class="col-md-3 text-vertical-align-center">' . $result->evaluation_types_name . '</div>';
                if ($right) {
                    $data .= ' <div class="col-md-3 text-vertical-align-center">' . $department_name . '</div>
                                <div class="col-md-3 text-vertical-align-center">' . $result->salary_grade_name . '</div>
                                <div class="col-md-2 text-vertical-align-center">';
                    if (hasRight($filters->L, 'edit')) {
                        $data .= '<a href="' . $admin_url . 'grading_policy?id=' . $result->id . '" class="btn btn-sm btn-light-primary font-weight-bolder text-uppercase mr-2" style="font-size: 10px" title="Edit">
                    ...
                    $data .= '<div class="col-md-4 text-vertical-align-center">' . $department_name . '</div>
                                <div class="col-md-4 text-vertical-align-center">' . $result->salary_grade_name . '</div>';
                }
                $data .= '</div>
                ...
                $query_gpd = mysqli_query($db, "SELECT * FROM `evaluation_grading_policy_details` WHERE `gp_id`='{$result->id}'");
                if (mysqli_num_rows($query_gpd) > 0) {
                    ...
                    $data .= '<div class="card-section-title mb-4">
                        <div class="card-section-title-inner">
                            <div class="row mb-2">
                                ...
                                <div class="col-md-2 column"><b>Title</b></div>
                                <div class="col-md-2 column"><b>Percentage</b> <small>(From)</small></div>
                                <div class="col-md-2 column"><b>Percentage</b> <small>(To)</small></div>
                                <div class="col-md-1 column"><b>Amount</b> <small>(From)</small></div>
                                <div class="col-md-1 column"><b>Amount</b> <small>(To)</small></div>
                                <div class="col-md-3 column"><b>Description</b></div>
                            </div>
                        </div>
                    </div>';
                    $sr = 0;
                    while ($result_gpd = mysqli_fetch_object($query_gpd)) {
                        $sr++;
                        $gp_action = empty($result_gpd->gp_action) ? '-' : $result_gpd->gp_action;
                        $data .= '<div class="card-section-body">
                            <div class="row mt-5">
                                <div class="col-md-1 column">' . $sr . '</div>
                                <div class="col-md-2 column">' . $result_gpd->gp_name . '</div>
                                <div class="col-md-2 column">' . $result_gpd->percentage_from . '%</div>
                                <div class="col-md-2 column">' . $result_gpd->percentage_to . '%</div>
                                <div class="col-md-1 column">' . number_format($result_gpd->amount_from, 2) . '</div>
                                <div class="col-md-1 column">' . number_format($result_gpd->amount_to, 2) . '</div>
                                <div class="col-md-3 column">' . $gp_action . '</div>
                            </div>
                        </div>';
                    }
                }
