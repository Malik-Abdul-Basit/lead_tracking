...
if (isset($_POST['filters']) && !empty($_POST['filters'])) {
    ...
    $total = 0;
    $data = '';
    $sql = mysqli_query($db, "SELECT count(id) AS total FROM `categories`" . $condition);
    ...
    if ($total > 0) {
        ...
        $select = "SELECT `id`,`name`,`sort_by` FROM `categories`" . $condition . $sort . $number_of_record;
                    ...
                    if (hasRight($filters->L, 'edit') || hasRight($filters->L, 'delete')) {
                        if (isset($filters->Actions) && !empty($filters->Actions) && sizeof($filters->Actions) > 0) {
                            ...
                            if (hasRight($filters->L, 'edit'))
                                $data .= '<a href="' . $admin_url . 'category?id=' . $result['id'] . '" class="btn btn-sm btn-clean btn-icon" title="Edit"><i class="la la-edit"></i></a>';
