...
if (isset($_POST['filters']) && !empty($_POST['filters'])) {
    ...
    $total = 0;
    $data = '';
    $sql = mysqli_query($db, "SELECT count(id) AS total FROM `evaluation_types`" . $condition);
    ...
    if ($total > 0) {
        ...
        $select = "SELECT `id`,`name`,`sort_by`,`duration` FROM `evaluation_types`" . $condition . $sort . $number_of_record;
        ...
        if (mysqli_num_rows($query) > 0) {
            ...
            while ($result = mysqli_fetch_assoc($query)) {
                ...
                foreach ($filters->Header as $key => $value) {
                    ...
                    if($f=='duration'){
                        $data .= '<span ' . $c_style . '>' . $result[$f] . ' Month</span>';
                    }
                    else{
                    ...
                    if (hasRight($filters->L, 'edit') || hasRight($filters->L, 'delete')) {
                        if (isset($filters->Actions) && !empty($filters->Actions) && sizeof($filters->Actions) > 0) {
                            ...
                            if (hasRight($filters->L, 'edit'))
                                $data .= '<a href="' . $admin_url . 'evaluation_type?id=' . $result['id'] . '" class="btn btn-sm btn-clean btn-icon" title="Edit"><i class="la la-edit"></i></a>';
