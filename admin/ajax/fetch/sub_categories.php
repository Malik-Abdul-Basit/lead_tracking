...
if (isset($_POST['filters']) && !empty($_POST['filters'])) {
    ...
    $sortColumn = 'sort_by';
    $sortOrder = 'ASC';
    $condition = " WHERE sc.company_id='{$global_company_id}' AND c.company_id='{$global_company_id}' AND sc.branch_id='{$global_branch_id}' AND c.branch_id='{$global_branch_id}' AND sc.deleted_at IS NULL AND c.deleted_at IS NULL ";
    if (isset($filters->SearchQuery) && !empty($filters->SearchQuery) && strlen($filters->SearchQuery) > 0) {
        $condition .= " AND (sc.id LIKE '%{$filters->SearchQuery}%' OR sc.name LIKE '%{$filters->SearchQuery}%' OR sc.sort_by LIKE '%{$filters->SearchQuery}%') ";
    }
    ...
    $total = 0;
    $data = '';
    $sql = mysqli_query($db, "SELECT count(sc.id) AS total FROM sub_categories AS sc INNER JOIN categories AS c ON sc.category_id = c.id " . $condition);
    ...
    if ($total > 0) {
        ...
        $select = "SELECT sc.id, sc.name, sc.sort_by,
        c.name AS category_name
        FROM
            sub_categories AS sc 
        INNER JOIN 
            categories AS c 
            ON sc.category_id = c.id " . $condition . " GROUP BY sc.id " . $sort . $number_of_record;
                    ...
                    if (hasRight($filters->L, 'edit') || hasRight($filters->L, 'delete')) {
                        if (isset($filters->Actions) && !empty($filters->Actions) && sizeof($filters->Actions) > 0) {
                            ...
                            if (hasRight($filters->L, 'edit'))
                                $data .= '<a href="' . $admin_url . 'sub_category?id=' . $result['id'] . '" class="btn btn-sm btn-clean btn-icon" title="Edit"><i class="la la-edit"></i></a>';
