...
if (isset($_POST['filters']) && !empty($_POST['filters'])) {
    ...
    $sortColumn = 'sort_by';
    $sortOrder = 'ASC';
    $condition = " WHERE t.company_id='{$global_company_id}' AND d.company_id='{$global_company_id}' AND t.branch_id='{$global_branch_id}' AND d.branch_id='{$global_branch_id}' AND t.deleted_at IS NULL AND d.deleted_at IS NULL ";
    if (isset($filters->SearchQuery) && !empty($filters->SearchQuery) && strlen($filters->SearchQuery) > 0) {
        $condition .= " AND (t.id LIKE '%{$filters->SearchQuery}%' OR t.name LIKE '%{$filters->SearchQuery}%' OR t.sort_by LIKE '%{$filters->SearchQuery}%') ";
    }
    ...
    $total = 0;
    $data = '';
    $sql = mysqli_query($db, "SELECT count(t.id) AS total FROM designations AS t INNER JOIN departments AS d ON t.department_id = d.id " . $condition);
    ...
    if ($total > 0) {
        ...
        $select = "SELECT
        t.id, t.name, t.report_to_designation_id, t.sort_by,
        b.name AS report_to_designation,
        d.name AS department_name, 
        sg.name AS salary_band_name,
        sgd.grade_name AS salary_grade_name 
            FROM 
                designations AS t 
            INNER JOIN
                departments AS d 
            ...
            INNER JOIN
                salary_grades AS sg
            ON t.salary_grade_id = sg.id
            INNER JOIN
                salary_grade_details AS sgd
            ON t.salary_grade_detail_id = sgd.id
            LEFT JOIN
                designations AS b
            ON t.report_to_designation_id = b.id " . $condition . " GROUP BY t.id " . $sort . $number_of_record;
        ...
        if (mysqli_num_rows($query) > 0) {
            ...
            while ($result = mysqli_fetch_assoc($query)) {
                ...
                foreach ($filters->Header as $key => $value) {
                    ...
                    $data .= '<span ' . $c_style . '>' . $result[$f] . '</span>';
                    $data .= '</td>';
                }
                if (isset($filters->Actions) && !empty($filters->Actions) && sizeof($filters->Actions) > 0) {
                    $ac = (object)$filters->Actions;
                    ...
                    $data .= '<span ' . $ac_style . '>';
                    $data .= '<a href="' . $admin_url . 'designation?id=' . $result['id'] . '" class="btn btn-sm btn-clean btn-icon" title="Edit"><i class="la la-edit"></i></a>
                                <button type="button" onclick="entryDelete(' . $result['id'] . ')" class="btn btn-sm btn-clean btn-icon" title="Delete"><i class="la la-trash"></i></button>';
                    $data .= '</span></td>';
                }
                $data .= '</tr>';
            }
