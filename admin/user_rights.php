...
?>
    <script>
        function saveFORM() {
            ...
            var select2_branch_id_container = document.querySelector("[aria-labelledby='select2-branch_id-container']");
            var employee_code = document.getElementById('employee_code');
            var A = '<?php echo hasRight($user_right_title, 'assign_rights') ?>';
            ...
            employee_code.style.borderColor = type.style.borderColor = status.style.borderColor = select2_branch_id_container.style.borderColor = '#E4E6EF';
            ...
            if (A == '') {
                ...
                select2_branch_id_container.style.borderColor = '#F00';
                error = "Branch field is required.";
                errorMessageBranch.innerText = error;
                toasterTrigger('error', error);
                return false;
            } else if (isNaN(branch_id.value) === true || branch_id.value < 1 || branch_id.value.length > 20) {
                select2_branch_id_container.style.borderColor = '#F00';
                error = "Please select a valid option of Branch.";
                errorMessageBranch.innerText = error;
                toasterTrigger('error', error);
                return false;
            } else {
                ...
                var postData = {
                    ...
                    "type": type.value,
                    "status": status.value,
                    "branch_id": JSON.parse("[" + branch_id.value + "]"),
