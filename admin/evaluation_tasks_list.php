                                    ...
                                    <div class="card-header flex-wrap py-5">
                                        ...
                                        <div class="card-toolbar">
                                            ...
                                            if (hasRight($user_right_title, 'add')) {
                                                echo '<a href="' . $admin_url . 'evaluation_tasks" class="btn btn-primary font-weight-bolder"><i class="la la-plus"></i>New Record</a>';
                                            }
                                            ?>
                                        </div>
                                    </div>
...
<script>
    ...
    function getData() {
        ...
        var filter = {
            ...
            'Filter': [
                {'field': 'edq.department_id', 'value': BG_DepartmentFilter.value},
                {'field': 'edq.designation_id', 'value': BG_DesignationFilter.value},
            ],
            "PageSizeStack": ["5", "10", "20", "30", "40", "50"]
        };
        getAllDefaultTasks(filter);
    }

    function getAllDefaultTasks(filter) {
        loader(true);
        $.ajax({
            type: "POST", url: 'ajax/fetch/default_tasks.php',
            data: {'filters': filter},
            ...
            if (result.value) {
                ...
                if (D) {
                    ...
                    $.ajax({
                        ...
                        data: "delete_default_evaluation_tasks=" + id,
                        success: function (resPonse) {
                            if (resPonse !== undefined && resPonse != '') {
                                var obj = JSON.parse(resPonse);
